...
class Party(
    DeactivableMixin, _ContactMechanismMixin, ModelSQL, ModelView, MultiValueMixin
):
    "Party"
    ...
    name = fields.Char("Name", strip=False, help="The main identifier of the party.")
    code = fields.Char(
        "Code",
        required=True,
        states={
    ...
    langs = fields.One2Many("party.party.lang", "party", "Languages")
    ...
    tax_identifier = fields.Function(
        fields.Many2One(
            "party.identifier",
            "Tax Identifier",
        ...
        "get_tax_identifier",
        searcher="search_tax_identifier",
    )
    ...
    def _search_identifier(cls, name, clause, types):
        ...
        if (operator == "=" and value is None) or (operator == "in" and None in value):
            domain = [
                "OR",
                domain,
                [
            ...
            if value:
                ...
                if not contact_mechanism:
                    contact_mechanism = ContactMechanism(
                        party=party, type=name, value=value
                    )
                if usage:
    ...
    def _distance_query(cls, usages=None, party=None, depth=None):
        ...
        table = cls.__table__()
        return table.select(
            table.id.as_("to"), Literal(0).as_("distance"), where=(table.id == party)
        )
    ...
    def order_distance(cls, tables):
        ...
        if key not in tables:
            ...
            join = party.join(query, type_="LEFT", condition=query.to == party.id)
            tables[key] = {
    ...
    def autocomplete(cls, text, domain=None, limit=None, order=None):
        ...
        result = super().autocomplete(text, domain=domain, limit=limit, order=order)
        ...
        for identifier in values.get("identifiers") or []:
            if identifier.get("type") in eu_types and identifier.get("code"):
                ...
                try:
                    ...
                    logger.debug(f"Fail to check {identifier['code']}", exc_info=True)
                    continue
                if result["valid"]:
...
IDENTIFIER_TYPES = [
    ...
    ("gb_nhs", "United Kingdom National Health Service Patient Identifier"),
    ...
    ("is_kennitala", "Icelandic Personal and Organisation Identity Code"),
    ...
    ("my_nric", "Malaysian National Registration Identity Card Number"),
    ...
    ("no_fodselsnummer", "Norwegian Birth Number, the National Identity Number"),
...
IDENTIFIER_TYPES = list(map(lambda x: (replace_vat(x[0]), x[1]), IDENTIFIER_TYPES))
...
class Identifier(sequence_ordered(), DeactivableMixin, ModelSQL, ModelView):
    ...
    def __register__(cls, module_name):
        ...
        for old in IDENTIFIER_VAT:
            new = replace_vat(old)
            cursor.execute(*table.update([table.type], [new], where=table.type == old))
...
class CheckVIESResult(ModelView):
    ...
    parties_succeed = fields.Many2Many(
        "party.party",
        None,
        None,
        "Parties Succeed",
    ...
    parties_failed = fields.Many2Many(
        "party.party",
        None,
        None,
        "Parties Failed",
                    ...
                    for msg in e.args:
                        if msg == "INVALID_INPUT":
                            ...
                            raise VIESUnavailable(
                                gettext("party.msg_vies_unavailable")
                            ) from e
                    else:
                        raise
...
class Replace(Wizard):
        ...
        if source.name != destination.name:
            ...
            if Warning.check(key):
                raise SimilarityWarning(
                    key,
                    gettext(
                        "party.msg_different_name",
                        source_name=source.name,
                        destination_name=destination.name,
                    ),
                )
        source_code = source.tax_identifier.code if source.tax_identifier else ""
        destination_code = (
            destination.tax_identifier.code if destination.tax_identifier else ""
        )
        if source_code != destination_code:
            key = "party.replace tax_identifier %s %s" % (source.id, destination.id)
            if Warning.check(key):
                raise SimilarityWarning(
                    key,
                    gettext(
    ...
    def transition_replace(self):
        ...
        ContactMechanism.write(
            list(source.contact_mechanisms),
            {
                "active": False,
            },
        )
        ...
        for model_name, field_name in self.fields_to_replace():
            ...
            cursor.execute(
                *table.update(
                    [column], [destination_value], where=where, returning=returning
                )
            )
...
class ReplaceAsk(ModelView):
    ...
    source = fields.Many2One(
        "party.party", "Source", required=True, help="The party to be replaced."
    )
...
class Erase(Wizard):
            ...
            for Model, domain, resource, columns, values in to_erase:
                ...
                if columns:
                    ...
                    for table in model_tables:
                        ...
                        cursor.execute(
                            *table.update(
                                sql_columns, sql_values, where=table.id.in_(query)
                            )
                        )
                if resource:
                    for Resource in resources:
                        model_tables = [(Resource.__table__(), Resource.resource)]
                        if Resource._history:
                            model_tables.append(
                                (Resource.__table_history__(), Resource.resource)
                            )
                        for table, resource_field in model_tables:
                            cursor.execute(
                                *table.delete(
                                    where=table.resource.like(Model.__name__ + ",%")
                                    & resource_field.sql_id(table.resource, Model).in_(
                                        query
                                    )
                                )
                            )
    ...
    def check_erase(self, party):
        if party.active:
            raise EraseError(
                gettext("party.msg_erase_active_party", party=party.rec_name)
            )
    ...
    def to_erase(self, party_id):
        ...
        ContactMechanism = pool.get("party.contact_mechanism")
        return [
            (Party, [("id", "=", party_id)], True, ["name"], [None]),
            (
                Identifier,
                [("party", "=", party_id)],
                True,
...
class EraseAsk(ModelView):
    ...
    party = fields.Many2One(
        "party.party", "Party", required=True, help="The party to be erased."
    )
